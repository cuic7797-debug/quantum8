...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    return Response.json({ status: 'ok', message: 'Quantum8 sync worker running' });
  },
};

async function syncDraws() {
  try {
    const data = await fetchCWL();
    if (!data?.result?.length) return { error: 'no data from CWL' };
    ...
    return { inserted, skipped, total: data.result.length };
  } catch (e) {
