...
// 建议用 Windows 任务计划程序每天 22:30 自动运行
...
async function main() {
  console.log(`[${new Date().toLocaleString()}] 开始同步...`);
  ...
  try {
    ...
    console.error('❌ 福彩官网请求失败:', e.message);
    process.exit(1);
  }
  ...
  console.log(`📥 获取到 ${data.result.length} 期数据`);

  // 2. Get existing
  const existing = await supabaseRequest('GET', '/rest/v1/draws?select=draw_number');
  ...
  for (const d of data.result) {
    ...
    await supabaseRequest('POST', '/rest/v1/draws', { draw_number: drawNumber, draw_date: dateStr, ...features });
    inserted++;
    console.log(`  ✅ ${drawNumber} (${dateStr})`);
  }

  console.log(`📊 新增 ${inserted} 期，跳过 ${skipped} 期`);

  // 4. Recalculate stats if needed
  if (inserted > 0) {
    console.log('🔄 更新号码统计...');
    const allDraws = await supabaseRequest('GET', '/rest/v1/draws?select=*&order=draw_date.desc&limit=200');
    if (Array.isArray(allDraws) && allDraws.length > 0) {
      await supabaseRequest('DELETE', '/rest/v1/number_stats?number=neq.0');
      const stats = [];
      ...
      for (let i = 0; i < stats.length; i += 20) {
        await supabaseRequest('POST', '/rest/v1/number_stats', stats.slice(i, i + 20));
      }
