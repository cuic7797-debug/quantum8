    ...
    <div className="glass-card p-5 space-y-4">
      ...
      {repeats.length > 0 && (
        <div className="glass-inset p-3">
          <div className="text-sm font-semibold text-[var(--color-muted)] mb-2">🔁 连续出现（重号延续）</div>
          <div className="space-y-1">
            {repeats.slice(0, 5).map(r => (
              <div key={r.num} className="flex items-center gap-2">
                ...
                <span className="text-sm font-mono text-[var(--color-muted)]">{r.streak}期连续</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {mirrorPairs.length > 0 && (
        <div className="glass-inset p-3">
          <div className="text-sm font-semibold text-[var(--color-muted)] mb-2">🪞 镜像对（和为81）</div>
          <div className="flex flex-wrap gap-2">
            {mirrorPairs.slice(0, 8).map(p => (
              <div key={p.a} className="flex items-center gap-1 glass-card px-2 py-1">
                ...
                <span className="text-xs text-[var(--color-primary)] ml-1">{p.coCount}次</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="glass-inset p-3">
        <div className="text-sm font-semibold text-[var(--color-muted)] mb-2">🎯 尾数聚类</div>
        <div className="grid grid-cols-5 gap-2">
          {topEndDigits.map(([digit, count]) => (
            <div key={digit} className="text-center glass-card p-2">
              ...
              <div className="text-xs text-[var(--color-muted)]">{count}期出现</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {longStreaks.length > 0 && (
        <div className="glass-inset p-3">
          <div className="text-sm font-semibold text-[var(--color-muted)] mb-2">⚖️ 奇偶连续偏态</div>
          <div className="flex flex-wrap gap-2">
            {longStreaks.map((s, i) => (
              <div key={i} className={'rounded-lg px-3 py-1.5 text-sm font-semibold ' + (
      ...
      <div className="text-xs text-[var(--color-muted)] text-center">模式识别基于近20期数据，仅供参考</div>
    </div>
  );
}
