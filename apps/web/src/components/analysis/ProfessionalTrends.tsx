          ...
          {recent.map(d => (
            <div key={d.draw_number} className="flex items-center gap-2">
              ...
              <div className="flex-1 flex h-5 rounded overflow-hidden">
                <div className="bg-blue-500 flex items-center justify-center text-[9px] text-white font-bold" style={{ width: (d.big_count / 20 * 100) + '%' }}>
                  {d.big_count > 5 ? d.big_count : ''}
                </div>
                <div className="bg-rose-500 flex items-center justify-center text-[9px] text-white font-bold" style={{ width: (d.small_count / 20 * 100) + '%' }}>
                  {d.small_count > 5 ? d.small_count : ''}
                </div>
              </div>
              <span className="text-[10px] font-mono w-12 text-right text-[var(--color-muted)]">{d.big_count}:{d.small_count}</span>
            </div>
          ))}
        ...
        <div className="space-y-3">
          ...
          {recent.map(d => (
            <div key={d.draw_number} className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-[var(--color-muted)] w-10">{d.draw_number.slice(-3)}</span>
              <div className="flex-1 flex h-5 rounded overflow-hidden">
                <div className="bg-purple-500 flex items-center justify-center text-[9px] text-white font-bold" style={{ width: (d.odd_count / 20 * 100) + '%' }}>
                  {d.odd_count > 5 ? d.odd_count : ''}
                </div>
                <div className="bg-cyan-500 flex items-center justify-center text-[9px] text-white font-bold" style={{ width: (d.even_count / 20 * 100) + '%' }}>
                  {d.even_count > 5 ? d.even_count : ''}
                </div>
              </div>
              <span className="text-[10px] font-mono w-12 text-right text-[var(--color-muted)]">{d.odd_count}:{d.even_count}</span>
            </div>
          ))}
          <div className="flex justify-between text-[10px] text-[var(--color-muted)]">
        ...
        <div className="space-y-3">
          ...
          {recent.map(d => (
            <div key={d.draw_number} className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-mono w-8 text-right text-[var(--color-muted)]">{d.span}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="space-y-3">
          ...
          {remainderTrend.map(r => (
            <div key={r.draw} className="flex items-center gap-1">
              <span className="text-[10px] font-mono text-[var(--color-muted)] w-8">{r.draw}</span>
              {r.rems.map((count, ri) => (
                <div key={ri} className="flex-1 text-center">
                  <div className="text-[9px] text-[var(--color-muted)]">余{ri}</div>
        ...
        <div className="space-y-3">
          ...
          {recent.map(d => (
            <div key={d.draw_number} className="flex items-center gap-1">
              ...
              <span className="text-[9px] font-mono w-20 text-right text-[var(--color-muted)]">{d.zone1_count}:{d.zone2_count}:{d.zone3_count}:{d.zone4_count}</span>
            </div>
          ))}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {topFreq.map(([num, count], i) => (
            <div key={num} className="flex items-center gap-2 glass-card px-2 py-1.5">
              ...
              <span className="text-[10px] font-mono font-bold">{count}次</span>
            </div>
          ))}
        </div>
      </div>
