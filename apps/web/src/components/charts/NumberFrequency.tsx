  ...
  return (
    <div className="glass-card p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-[10px] text-[var(--color-muted)]">{total}期数据</span>
      </div>
        ...
        {items.map(([num, count]: [number, number], i: number) => {
          ...
          return (
            <div key={num} className="flex items-center gap-2 group">
              ...
              <span className="w-7 h-7 rounded-full bg-[var(--color-primary)]/15 text-[var(--color-primary)] flex items-center justify-center text-xs font-bold font-mono shrink-0">
              ...
              <div className="flex-1 h-5 glass-inset rounded overflow-hidden relative">
                <div className="h-full bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-accent)] rounded transition-all duration-500"
                  style={{ width: `${pct}%` }} />
                <span className="absolute inset-0 flex items-center justify-end pr-2 text-[9px] font-mono text-[var(--color-muted)] group-hover:text-[var(--color-text)] transition-colors">
                  {count}次 ({rate}%)
                </span>
              </div>
            </div>
