...
import { generateRandomCombination, applyFilters, scoreCombination } from '@quantum8/algorithm';
  ...
  function handleBacktest() {
    ...
    setTimeout(() => {
      const config: StrategyConfig = {
        hotCount: 5, coldCount: 3, balanceCount: 2,
        zoneBalance: true, sumRange: [400, 1200], oddEvenRange: [5, 15], maxConsecutive: 3,
      };
      const table = PRIZE_TABLE[playType] || {};
      const testDraws = draws.slice(1);
      const rows: BacktestRow[] = [];
      let totalHits = 0, totalPrize = 0, maxPrize = 0;
      ...
      for (let i = 0; i < testDraws.length; i++) {
        ...
        if (i < 30) {
          rows.push({ period: testDraws[i].draw_number, numbers: target, hitCount: bestHit, prize });
        }
      }
      ...
      setResult({
        totalRounds: testDraws.length,
        totalBets,
        totalHits,
        hitRate: parseFloat(((totalHits / testDraws.length) * 100).toFixed(2)),
        totalCost,
        totalPrize,
        roi: totalCost > 0 ? parseFloat((((totalPrize - totalCost) / totalCost) * 100).toFixed(2)) : 0,
        maxPrize,
        rows,
      });
      setRunning(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-xs text-[var(--color-muted)] bg-amber-500/10 border border-amber-500/20 rounded-lg px-4 py-2">
        ⚠️ 回测结果基于历史数据模拟，不代表未来表现，仅供参考。
      </div>

      {/* 设置 */}
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5 space-y-4">
        <div>
          <h3 className="text-sm font-semibold text-[var(--color-muted)] mb-2">玩法</h3>
          <div className="flex flex-wrap gap-2">
            {PLAY_TYPES.map((pt) => (
              <button key={pt} onClick={() => setPlayType(pt)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  playType === pt ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg)] text-[var(--color-muted)] hover:text-white'
                }`}>
                {pt}
              </button>
            ))}
        ...
        <div>
          <h3 className="text-sm font-semibold text-[var(--color-muted)] mb-2">每期注数</h3>
          <div className="flex gap-2">
            {[1, 3, 5, 10].map((n) => (
              <button key={n} onClick={() => setBetCount(n)}
                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  betCount === n ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg)] text-[var(--color-muted)] hover:text-white'
                }`}>
                {n}注
              </button>
            ))}
        ...
        <button onClick={handleBacktest} disabled={running || draws.length < 10}
          ...
          {running ? '回测中...' : `开始回测（最近 ${Math.min(draws.length - 1, 500)} 期）`}
        </button>
      </div>
      ...
      {result && (
        <>
          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
            <h3 className="text-sm font-semibold text-[var(--color-muted)] mb-4">回测结果</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
                <div className="text-xl font-bold">{result.totalRounds}</div>
                <div className="text-xs text-[var(--color-muted)]">测试期数</div>
              </div>
              <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
                <div className="text-xl font-bold">{result.hitRate}%</div>
                <div className="text-xs text-[var(--color-muted)]">命中率</div>
              </div>
              <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
                <div className="text-xl font-bold font-mono">¥{result.totalCost}</div>
                <div className="text-xs text-[var(--color-muted)]">总投入</div>
              </div>
              <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
                <div className={`text-xl font-bold font-mono ${result.roi >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  {result.roi >= 0 ? '+' : ''}{result.roi}%
                </div>
                <div className="text-xs text-[var(--color-muted)]">收益率</div>
              </div>
            </div>
          </div>

          <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
            <h3 className="text-sm font-semibold text-[var(--color-muted)] mb-3">近期明细</h3>
              ...
              {result.rows.map((row, i) => (
                <div key={i} className="flex items-center justify-between py-2 border-b border-[var(--color-border)] last:border-0 text-sm">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`font-mono font-bold ${row.hitCount >= 5 ? 'text-emerald-400' : row.hitCount >= 3 ? 'text-amber-400' : 'text-[var(--color-muted)]'}`}>
                      中{row.hitCount}
                    </span>
