...
export default function HistoryPage() {
  ...
  function exportPicks() {
    ...
    const lines = picks.map(p => `[${p.time}] ${p.playType} | ${p.numbers.join(' ')} | Score:${p.score} ${p.risk}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab('check')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'check' ? 'bg-[var(--color-primary)] text-white shadow' : 'bg-[var(--color-surface)] text-[var(--color-muted)] hover:bg-[var(--color-border)]'}`}>{t('check_numbers')}</button>
        <button onClick={() => setTab('records')} className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'records' ? 'bg-[var(--color-primary)] text-white shadow' : 'bg-[var(--color-surface)] text-[var(--color-muted)] hover:bg-[var(--color-border)]'}`}>{t('my_picks')} ({picks.length})</button>
      </div>

      {tab === 'check' && (
        <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5 space-y-4">
          <div><label className="text-sm text-[var(--color-muted)] mb-1 block">{t('select_draw')}</label>
            <select value={sd} onChange={e => setSd(e.target.value)} className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm">
          ...
          <div><label className="text-sm text-[var(--color-muted)] mb-1 block">{t('enter_numbers')}</label>
            <input type="text" value={cn} onChange={e => setCn(e.target.value)} placeholder="1 5 12 23 34 45 56 67 78 80" className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm font-mono" />
          </div>
          <button onClick={go} className="w-full py-2.5 rounded-xl bg-[var(--color-primary)] text-white font-semibold hover:bg-[var(--color-primary)]/80 transition-all shadow">{t('check')}</button>

          {cr && (
            <div className="bg-[var(--color-bg)] rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="text-sm">{cr.d}</div>
                <div className={`text-lg font-bold ${cr.h>=5?'text-emerald-400':cr.h>=3?'text-amber-400':'text-[var(--color-muted)]'}`}>{t('hit_count')}: {cr.h} {t('pcs')}</div>
              </div>
      ...
      {tab === 'records' && (
        <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
          {picks.length === 0 ? (
            <div className="text-center py-8 text-[var(--color-muted)] text-sm">{t('no_records')}<br/><span className="text-xs">在「智能选号」页面生成号码后点击「保存」</span></div>
          ) : (
            <>
                  ...
                  <div key={i} className="flex items-center justify-between py-3 border-b border-[var(--color-border)] last:border-0">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded ${p.risk==='低'?'bg-emerald-500/20 text-emerald-400':p.risk==='中'?'bg-amber-500/20 text-amber-400':'bg-red-500/20 text-red-400'}`}>{p.risk}风险</span>
                      </div>
