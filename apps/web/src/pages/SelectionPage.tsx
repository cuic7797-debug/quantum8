...
const STRATEGIES = [
  { name: '稳健模式', hot: 4, cold: 4, balance: 2, desc: '均衡冷热号，适合保守投注' },
  { name: '均衡模式', hot: 6, cold: 3, balance: 1, desc: '偏热号为主，兼顾冷号回补' },
  { name: '激进模式', hot: 8, cold: 1, balance: 1, desc: '追热号趋势，高风险高回报' },
];
...
export default function SelectionPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-xs text-[var(--color-muted)] bg-amber-500/10 border border-amber-500/20 rounded-lg px-4 py-2">⚠️ 以下结果基于历史数据统计分析，仅供参考，不构成投注建议。</div>
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
        <h3 className="text-sm font-semibold text-[var(--color-muted)] mb-3">第一步：选择玩法</h3>
        <div className="flex flex-wrap gap-2">
          {PLAY_TYPES.map((pt) => (
            <button key={pt} onClick={() => setPlayType(pt)} className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${playType === pt ? 'bg-[var(--color-primary)] text-white' : 'bg-[var(--color-bg)] text-[var(--color-muted)] hover:text-white'}`}>{pt}</button>
          ))}
        </div>
      </div>
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {STRATEGIES.map((s, i) => (
            <button key={s.name} onClick={() => setStrategyIdx(i)} className={`text-left p-4 rounded-xl border transition-all ${strategyIdx === i ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/10' : 'border-[var(--color-border)] hover:border-[var(--color-primary)]/50'}`}>
      ...
      <button onClick={handleGenerate} disabled={generating || stats.length === 0} className="w-full py-3 rounded-xl bg-[var(--color-primary)] text-white font-semibold text-lg hover:bg-[var(--color-primary)]/80 disabled:opacity-50 transition-colors">
        {generating ? '生成中...' : `生成 ${playType} 推荐`}
      </button>
      {results.length > 0 && (
        <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
          <h3 className="text-sm font-semibold text-[var(--color-muted)] mb-4">AI 推荐 {playType}</h3>
          <div className="space-y-3">
            {results.map((r, i) => (
              <div key={i} className="flex items-center justify-between py-3 border-b border-[var(--color-border)] last:border-0">
                <div>
                  <div className="flex items-center gap-2 mb-1"><span className="text-xs text-[var(--color-muted)]">#{i + 1}</span><div className="flex gap-1">{r.numbers.map((n) => <NumberBall key={n} number={n} size="sm" />)}</div></div>
                  <div className="flex gap-3 text-[10px] text-[var(--color-muted)]">{r.reasons.map((reason, ri) => <span key={ri}>✓ {reason}</span>)}</div>
                </div>
