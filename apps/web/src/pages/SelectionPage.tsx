...
export default function SelectionPage() {
  ...
  function handleGenerate() {
    ...
    setTimeout(() => {
      ...
      const config = {
        hotCount: strat.hot,
        coldCount: strat.cold,
        balanceCount: strat.balance,
        zoneBalance: true,
        sumRange: [400, 1200] as [number, number],
        oddEvenRange: [5, 15] as [number, number],
        maxConsecutive: 3,
      };
      ...
      const scored = filtered
        .slice(0, 50)
        .map((c) => scoreCombination(c, stats, draws.length))
              ...
              <div key={i} className="flex items-center justify-between py-3 border-b border-[var(--color-border)] last:border-0">
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className={`text-xs font-medium ${
                    r.riskLevel === '低' ? 'text-emerald-400' : r.riskLevel === '中' ? 'text-amber-400' : 'text-red-400'
                  }`}>
                    {r.riskLevel}风险
                  </div>
                </div>
