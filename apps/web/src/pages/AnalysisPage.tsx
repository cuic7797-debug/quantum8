...
export default function AnalysisPage() {
  ...
  // 连号统计
  let has2Consecutive = 0, has3Plus = 0, noConsecutive = 0;
  draws.forEach((d) => {
    ...
    if (maxStreak >= 3) has3Plus++;
    else if (maxStreak >= 2) has2Consecutive++;
    else noConsecutive++;
  });

  // 奇偶分布趋势（近15期）
  const oddEvenTrend = draws.slice(0, 15).map((d) => ({
    draw: d.draw_number.slice(-3),
    odd: d.odd_count,
    even: d.even_count,
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
            <div className="text-2xl font-bold">{has2Consecutive}</div>
            <div className="text-xs text-[var(--color-muted)]">含2连号</div>
            <div className="text-xs text-[var(--color-muted)]">{((has2Consecutive / draws.length) * 100).toFixed(1)}%</div>
          </div>
          <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
            <div className="text-2xl font-bold">{has3Plus}</div>
            <div className="text-xs text-[var(--color-muted)]">含3连号+</div>
            <div className="text-xs text-[var(--color-muted)]">{((has3Plus / draws.length) * 100).toFixed(1)}%</div>
          </div>
          <div className="text-center bg-[var(--color-bg)] rounded-lg p-3">
            <div className="text-2xl font-bold">{noConsecutive}</div>
            <div className="text-xs text-[var(--color-muted)]">无连号</div>
            <div className="text-xs text-[var(--color-muted)]">{((noConsecutive / draws.length) * 100).toFixed(1)}%</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
        ...
        <div className="flex justify-between mt-2 text-[10px] text-[var(--color-muted)]">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-blue-500 inline-block" /> 奇数</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-rose-500 inline-block" /> 偶数</span>
        </div>
      </div>
      ...
      <div className="bg-[var(--color-surface)] rounded-xl border border-[var(--color-border)] p-5">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
          {[...stats]
            .sort((a, b) => b.currentMiss - a.currentMiss)
            .slice(0, 20)
            .map((s, i) => (
